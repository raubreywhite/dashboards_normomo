...
SMAOFormatGGPlot <- function(q, sizeMultiplier = 3, legendKey = 3, xAngle = 0, stripes = TRUE, legendPos = "bottom", ncol = 3, legendBorder = FALSE, reverse = FALSE) {
  ...
  q <- q + theme(axis.line.x = element_line(
    colour = "black",
    size = 0.5 * sizeMultiplier
  ))
  q <- q + theme(axis.line.y = element_line(
    colour = "black",
    size = 0.5 * sizeMultiplier
  ))
  ...
  q <- q + theme(axis.title.x = element_text(
    size = 10 * sizeMultiplier,
    vjust = 0, colour = "black"
  ))
  q <- q + theme(axis.title.y = element_text(
    size = 10 * sizeMultiplier,
    angle = 90, vjust = 0.25, colour = "black"
  ))
  q <- q + theme(axis.text.y = element_text(
    size = 10 * sizeMultiplier,
    hjust = 1, vjust = 0.4, colour = "black"
  ))
  q <- q + theme(axis.text.x = element_text(
    size = 10 * sizeMultiplier,
  ...
  q <- q + theme(strip.text.y = element_text(
    size = 10 * sizeMultiplier,
    hjust = 0.5, colour = "black"
  ))
  q <- q + theme(strip.text.x = element_text(
    size = 10 * sizeMultiplier,
    hjust = 0.5, colour = "black"
  ))
  q <- q + theme(legend.text = element_text(
    size = 10 * sizeMultiplier,
    hjust = 0.5, colour = "black"
  ))
  q <- q + theme(legend.title = element_text(
    size = 10 * sizeMultiplier,
    hjust = 0.5, colour = "black"
  ))
  q <- q + theme(legend.position = "right")
  q <- q + theme(plot.margin = unit(c(0.5, 0.5, 1, 0.5), "lines"))
  q <- q + theme(plot.title = element_text(
...
RunTemporaryGraphs <- function(runName = "Norway",
  ...
  for (i in runList) {
    ...
    gridExtra::grid.arrange(
      ...
      xGraphHistoric(data, title = title2, includeRealDeaths = FALSE),
      ncol = 1
    )
